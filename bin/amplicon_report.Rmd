...
if (any(grepl("_primer_contam_stats.txt", list.files()))) {
    ...
    primer_contam_files <- list.files(
        pattern = "^asv.*primer_contam_stats\\.txt$"
    )
    
    db_types <- c()
    for (i in primer_contam_files) {
        db_types <- c(db_types, strsplit(i, split = "_")[[1]][2])
    }
    db_types <- unique(db_types)[1]

    if (file.exists(paste0("asv_", db_types, "_primer_contam_stats.txt"))) {
        primer_info           <- readLines(paste0("asv_", db_types, "_primer_contam_stats.txt")) %>%
                                data.frame()
        primer_infos[["ASV"]] <- list(primer_info)
    }

    if (file.exists(paste0("zotu_", db_types, "_primer_contam_stats.txt"))) {
        primer_info           <- readLines(paste0("zotu_", db_types, "_primer_contam_stats.txt")) %>%
                                data.frame()
...
# TAXA
########################################
if (any(grepl("phyloseq_taxa", list.files()))) {
    taxa_bool      <- TRUE
    tax_dfs        <- list()
    na_dfs         <- list()
    both_dfs       <- list()
    ...
    taxa_files <- list.files(
        pattern = "^asv.*phyloseq_taxa_filtered\\.tsv$"
    )
    ...
    db_types <- unique(db_types)
    db_one <- db_types[1]
    if (length(db_types) == 2) {
        db_two <- db_types[2]
        db_two_exists <- TRUE
    } else {
        db_two <- FALSE
    ...
    if (file.exists(paste0("asv_", db_one, "_phyloseq_taxa_filtered.tsv"))) {
        seq_type            <- "ASV"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table(paste0("asv_", db_one, "_phyloseq_taxa_filtered.tsv"),
                                        sep = "\t", header = TRUE)
        ...
        both_dfs[[seq_type]] <- list(data.frame(curr_taxa))

    } else if (file.exists(paste0("asv_", db_one, "_phyloseq_taxa.tsv"))) {
        seq_type            <- "ASV"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table(paste0("asv_", db_one, "_phyloseq_taxa.tsv"),
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists(paste0("asv_", db_one, "_lulucurated_phyloseq_taxa_filtered.tsv"))) {
        seq_type            <- "ASV_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table(paste0("asv_", db_one, "_lulucurated_phyloseq_taxa_filtered.tsv"),
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists(paste0("asv_", db_one, "_lulucurated_phyloseq_taxa.tsv"))) {
        seq_type            <- "ASV_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table(paste0("asv_", db_one, "_lulucurated_phyloseq_taxa.tsv"),
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists(paste0("zotu_", db_one, "_phyloseq_taxa_filtered.tsv"))) {
        seq_type            <- "ZOTU"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table(paste0("zotu_", db_one, "_phyloseq_taxa_filtered.tsv"),
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists(paste0("zotu_", db_one, "_phyloseq_taxa.tsv"))) {
        seq_type            <- "ZOTU"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table(paste0("zotu_", db_one, "_phyloseq_taxa.tsv"),
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists(paste0("zotu_", db_one, "_lulucurated_phyloseq_taxa_filtered.tsv"))) {
        seq_type            <- "ZOTU_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table(paste0("zotu_", db_one, "_lulucurated_phyloseq_taxa_filtered.tsv"),
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists(paste0("zotu_", db_one, "_lulucurated_phyloseq_taxa.tsv"))) {
        seq_type            <- "ZOTU_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table(paste0("zotu_", db_one, "_lulucurated_phyloseq_taxa.tsv"),
                                        sep = "\t", header = TRUE)
    ...
    if (db_two_exists) {
        if (file.exists(paste0("asv_", db_two, "_phyloseq_taxa_filtered.tsv"))) {
            seq_type            <- "ASV_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("asv_", db_two, "_phyloseq_taxa_filtered.tsv"),
                                            sep = "\t", header = TRUE)
        ...
        } else if (file.exists(paste0("asv_", db_two, "_phyloseq_taxa.tsv"))) {
            seq_type            <- "ASV_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("asv_", db_two, "_phyloseq_taxa.tsv"),
                                            sep = "\t", header = TRUE)
        ...
        if (file.exists(paste0("asv_", db_two, "_lulucurated_phyloseq_taxa_filtered.tsv"))) {
            seq_type            <- "ASV_LULUCURATED_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("asv_", db_two, "_lulucurated_phyloseq_taxa_filtered.tsv"),
                                            sep = "\t", header = TRUE)
        ...
        } else if (file.exists(paste0("asv_", db_two, "_lulucurated_phyloseq_taxa.tsv"))) {
            seq_type            <- "ASV_LULUCURATED_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("asv_", db_two, "_lulucurated_phyloseq_taxa.tsv"),
                                            sep = "\t", header = TRUE)
        ...
        if (file.exists(paste0("zotu_", db_two, "_phyloseq_taxa_filtered.tsv"))) {
            seq_type            <- "ZOTU_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("zotu_", db_two, "_phyloseq_taxa_filtered.tsv"),
                                            sep = "\t", header = TRUE)
        ...
        } else if (file.exists(paste0("zotu_", db_two, "_phyloseq_taxa.tsv"))) {
            seq_type            <- "ZOTU_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("zotu_", db_two, "_phyloseq_taxa.tsv"),
                                            sep = "\t", header = TRUE)
        ...
        if (file.exists(paste0("zotu_", db_two, "_lulucurated_phyloseq_taxa_filtered.tsv"))) {
            seq_type            <- "ZOTU_LULUCURATED_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("zotu_", db_two, "_lulucurated_phyloseq_taxa_filtered.tsv"),
                                            sep = "\t", header = TRUE)
        ...
        } else if (file.exists(paste0("zotu_", db_two, "_lulucurated_phyloseq_taxa.tsv"))) {
            seq_type            <- "ZOTU_LULUCURATED_SECOND_DB"
            analysis_types      <- append(analysis_types, seq_type)
            curr_taxa           <- read.table(paste0("zotu_", db_two, "_lulucurated_phyloseq_taxa.tsv"),
                                            sep = "\t", header = TRUE)
    ...
    for (i in analysis_types) {
        ...
        species[[i]] <- paste0(species[[i]], " (", round((species[[i]]/asvs_num[[i]])*100, 2), "%)")
        genus[[i]]   <- paste0(genus[[i]], " (", round((genus[[i]]/asvs_num[[i]])*100, 2), "%)")
        family[[i]]  <- paste0(family[[i]], " (", round((family[[i]]/asvs_num[[i]])*100, 2), "%)")
        order[[i]]   <- paste0(order[[i]], " (", round((order[[i]]/asvs_num[[i]])*100, 2), "%)")
        class[[i]]   <- paste0(class[[i]], " (", round((class[[i]]/asvs_num[[i]])*100, 2), "%)")
        phylum[[i]]  <- paste0(phylum[[i]], " (", round((phylum[[i]]/asvs_num[[i]])*100, 2), "%)")
        domain[[i]]  <- paste0(domain[[i]], " (", round((domain[[i]]/asvs_num[[i]])*100, 2), "%)")
        nas[[i]]     <- paste0(nas[[i]], " (", round((nas[[i]]/asvs_num[[i]])*100, 2), "%)")
    }
} else {
    taxa_bool      <- FALSE
    seq_types      <- c()
...
`r if (missing_bool) "## Missing samples"`

`r if (missing_bool) "Discarded samples and samples with no reads assigned after demultiplexing"`

`r if (missing_bool) knitr::kable(missing_samples, align="l")`
...
title_primer     <- "Primer sequences leftover after demultiplexing and trimming"
...
`r if (sample_type_exists) "## Statistics of different sample types"`

`r if (sample_type_exists) "Counts of different sample types"`

`r if (sample_type_exists) knitr::kable(type_counts_df, align="l")`

`r if (sample_type_exists) "5-number summary of the read counts for different sample types"`

`r if (sample_type_exists) knitr::kable(type_5numsum_df, align="l")`

`r if (sample_type_exists) "Mean of the read counts for different sample types"`

`r if (sample_type_exists) knitr::kable(type_mean_df, align="l")`

`r if (sample_type_exists) "\\\n\\"`
...
if (sample_type_exists) {
    text_density      <- c(
        ...
        scale_x_continuous(name='number_sequences', labels = comma) +
        scale_y_continuous(name='number_samples', labels = comma)",
        "```  \n"
    )
} else {
    text_density      <- c(
...
title_track     <- "Tracking read fate throughout demultiplexing and primer trimming"
subtitle_track  <- "Lets look at the allocation of demultiplexed reads compared to the raw data."
text_track      <- c(
    "```{r intext_track_reads, results='asis', echo=FALSE}\n",
    "reads_tracking %>%
        ggplot(aes(x=factor(Stage, level = c('Before Demux', 'After Demux', 'After Primer Trimming')), y=Reads, fill=Type, label=scales::comma(Reads))) +
...
title_errors     <- "Error rates informing ASV inference"
...
title_qual     <- "Selected sample quality profiles"
...
title_distribution     <- "ASV sequence length distribution"
...
title_stages     <- "Read retention through different workflow stages"
subtitle_stages  <- "Now lets look at box plots that visualise the number of reads retained after the different steps of the pipeline."
text_stages      <- c(
    "### Analysis Type: {{i}}  \n",
...
title_lca     <- "Summary of taxonomic identity assignment"
if ("ASV" %in% seq_types & ! "ZOTU" %in% seq_types) {
    subtitle_lca  <- "The number of ASVs with a taxonomic assignment (Lowest Common Ancestor; LCAs) at different levels of the taxonomic heirarchy"
    text_lca      <- c(
        "### Analysis Type: {{i}}  \n",
        "```{r intext_lca_{{i}}, results='asis', echo=FALSE}  \n",
        "cat('  \n',
        'ASVs with LCAs at species level: ', species[['{{i}}']], '  \n',
        'ASVs with LCAs at genus level: ', genus[['{{i}}']], '  \n',
        'ASVs with LCAs at family level: ', family[['{{i}}']], '  \n',
        'ASVs with LCAs at order level: ', order[['{{i}}']], '  \n',
        'ASVs with LCAs at class level: ', class[['{{i}}']], '  \n',
        'ASVs with LCAs at phylum level: ', phylum[['{{i}}']], '  \n',
        'ASVs with LCAs at domain level: ', domain[['{{i}}']], '  \n',
        'No assignments: ', nas[['{{i}}']], '  \n',
        '  \n')",
        "```  \n"
    )
} else if ("ZOTU" %in% seq_types & ! "ASV" %in% seq_types) {
    subtitle_lca  <- "The number of ZOTUs with a taxonomic assignment (Lowest Common Ancestor; LCAs) at different levels of the taxonomic heirarchy"
    text_lca      <- c(
        "### Analysis Type: {{i}}  \n",
        "```{r intext_lca_{{i}}, results='asis', echo=FALSE}  \n",
        "cat('  \n',
        'ZOTUs with LCAs at species level: ', species[['{{i}}']], '  \n',
        'ZOTUs with LCAs at genus level: ', genus[['{{i}}']], '  \n',
        'ZOTUs with LCAs at family level: ', family[['{{i}}']], '  \n',
        'ZOTUs with LCAs at order level: ', order[['{{i}}']], '  \n',
        'ZOTUs with LCAs at class level: ', class[['{{i}}']], '  \n',
        'ZOTUs with LCAs at phylum level: ', phylum[['{{i}}']], '  \n',
        'ZOTUs with LCAs at domain level: ', domain[['{{i}}']], '  \n',
        'No assignments: ', nas[['{{i}}']], '  \n',
        '  \n')",
        "```  \n"
    )
} else {
    subtitle_lca  <- "The number of ASVs/ZOTUs with a taxonomic assignment (Lowest Common Ancestor; LCAs) at different levels of the taxonomic heirarchy"
    text_lca      <- c(
        "### Analysis Type: {{i}}  \n",
        "```{r intext_lca_{{i}}, results='asis', echo=FALSE}  \n",
        "cat('  \n',
        'ASVs/ZOTUs with LCAs at species level: ', species[['{{i}}']], '  \n',
        'ASVs/ZOTUs with LCAs at genus level: ', genus[['{{i}}']], '  \n',
        'ASVs/ZOTUs with LCAs at family level: ', family[['{{i}}']], '  \n',
        'ASVs/ZOTUs with LCAs at order level: ', order[['{{i}}']], '  \n',
        'ASVs/ZOTUs with LCAs at class level: ', class[['{{i}}']], '  \n',
        'ASVs/ZOTUs with LCAs at phylum level: ', phylum[['{{i}}']], '  \n',
        'ASVs/ZOTUs with LCAs at domain level: ', domain[['{{i}}']], '  \n',
        'No assignments: ', nas[['{{i}}']], '  \n',
        '  \n')",
        "```  \n"
    )
