...
if (any(grepl("_primer_contam_stats.txt", list.files()))) {
    ...
    if (file.exists("asv_blast_primer_contam_stats.txt")) {
        primer_info           <- readLines("asv_blast_primer_contam_stats.txt") %>%
                                data.frame()
        primer_infos[["ASV"]] <- list(primer_info)
    }

    if (file.exists("zotu_blast_primer_contam_stats.txt")) {
        primer_info           <- readLines("zotu_blast_primer_contam_stats.txt") %>%
                                data.frame()
...
if (any(grepl("phyloseq_taxa", list.files()))) {
    ...
    if (file.exists("asv_blast_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ASV"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table("asv_blast_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("asv_blast_phyloseq_taxa.tsv")) {
        seq_type            <- "ASV"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table("asv_blast_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists("asv_blast_lulucurated_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ASV_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("asv_blast_lulucurated_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("asv_blast_lulucurated_phyloseq_taxa.tsv")) {
        seq_type            <- "ASV_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("asv_blast_lulucurated_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists("zotu_blast_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ZOTU"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table("zotu_blast_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("zotu_blast_phyloseq_taxa.tsv")) {
        seq_type            <- "ZOTU"
        analysis_types      <- append(analysis_types, seq_type)
        seq_types           <- append(seq_types, seq_type)
        curr_taxa           <- read.table("zotu_blast_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists("zotu_blast_lulucurated_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ZOTU_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("zotu_blast_lulucurated_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("zotu_blast_lulucurated_phyloseq_taxa.tsv")) {
        seq_type            <- "ZOTU_LULUCURATED"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("zotu_blast_lulucurated_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists("asv_blast2_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ASV_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("asv_blast2_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("asv_blast2_phyloseq_taxa.tsv")) {
        seq_type            <- "ASV_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("asv_blast2_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists("asv_blast2_lulucurated_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ASV_LULUCURATED_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("asv_blast2_lulucurated_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("asv_blast2_lulucurated_phyloseq_taxa.tsv")) {
        seq_type            <- "ASV_LULUCURATED_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("asv_blast2_lulucurated_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists("zotu_blast2_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ZOTU_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("zotu_blast2_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("zotu_blast2_phyloseq_taxa.tsv")) {
        seq_type            <- "ZOTU_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("zotu_blast2_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
    ...
    if (file.exists("zotu_blast2_lulucurated_phyloseq_taxa_filtered.tsv")) {
        seq_type            <- "ZOTU_LULUCURATED_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("zotu_blast2_lulucurated_phyloseq_taxa_filtered.tsv",
                                        sep = "\t", header = TRUE)
    ...
    } else if (file.exists("zotu_blast2_lulucurated_phyloseq_taxa.tsv")) {
        seq_type            <- "ZOTU_LULUCURATED_SECOND_DB"
        analysis_types      <- append(analysis_types, seq_type)
        curr_taxa           <- read.table("zotu_blast2_lulucurated_phyloseq_taxa.tsv",
                                        sep = "\t", header = TRUE)
