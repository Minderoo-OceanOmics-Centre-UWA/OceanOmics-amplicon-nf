process ALT_POSTDEMUX_SAMPSHEET {
    tag "$samplesheet"
    label 'process_low'
    ...
    for row in range(len(sampsheet.index)):
        ...
        if not os.path.isfile(os.getcwd() + "/demuxed_" + str(sampsheet.at[row, "samp_name"]) + ".R1.fq.gz"):
            missing_samples.append(sampsheet.at[row, "samp_name"])
            drop_rows.append(row)
        elif (os.stat(os.getcwd() + "/demuxed_" + str(sampsheet.at[row, "samp_name"]) + ".R1.fq.gz").st_size <= 25):
            missing_samples.append(sampsheet.at[row, "samp_name"])
            drop_rows.append(row)
        else:
            sampsheet.at[row, "fastq_1"] = os.getcwd() + "/demuxed_" + str(sampsheet.at[row, "samp_name"]) + ".R1.fq.gz"
            sampsheet.at[row, "fastq_2"] = os.getcwd() + "/demuxed_" + str(sampsheet.at[row, "samp_name"]) + ".R2.fq.gz"
